...
Each app that requires configuration has a template file ending in `.example` in the `apps/` directory:

| Template | Actual Config | Purpose |
|----------|--------------|---------|
| `apps/config.py.example` | `apps/config.py` | QRZ.com credentials |
| `apps/claude_config.json.example` | `apps/claude_config.json` | Claude AI settings |
| `apps/gemini_config.json.example` | `apps/gemini_config.json` | Gemini AI settings |
| `apps/smtp_config.json.example` | `apps/smtp_config.json` | Email relay settings |
| `apps/wiki_config.json.example` | `apps/wiki_config.json` | ZIM file paths |
| `apps/blog-app/blog_config.json.example` | `apps/blog-app/blog_config.json` | Blog database |
...
```bash
# Example: QRZ config
cp apps/config.py.example apps/config.py
nano apps/config.py
```
...
cd ~/.local/share/emcomm-tools/bbs-server/bbs-server-apps
sudo ./install.sh --uninstall

# Remove repository
cd ~/.local/share/emcomm-tools/bbs-server
rm -rf bbs-server-apps
```
